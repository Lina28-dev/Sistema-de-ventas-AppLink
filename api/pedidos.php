...
function listarPedidos($pdo) {
    // Verificar si la tabla fs_pedidos existe
    $tableExists = $pdo->query("SHOW TABLES LIKE 'fs_pedidos'")->rowCount() > 0;
...
function obtenerEstadisticas($pdo) {
    ...
    $stats['hoy'] = $pdo->query("SELECT COUNT(*) FROM fs_pedidos WHERE DATE(fecha_pedido) = CURDATE()")->fetchColumn();
