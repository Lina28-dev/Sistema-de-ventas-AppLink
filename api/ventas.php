...
function listarVentas($pdo) {
    // Verificar si la tabla fs_ventas existe
    $tableExists = $pdo->query("SHOW TABLES LIKE 'fs_ventas'")->rowCount() > 0;
...
function obtenerEstadisticas($pdo) {
    ...
    $stats['total'] = (int)$pdo->query("SELECT COUNT(*) FROM fs_ventas WHERE estado = 'completada'")->fetchColumn();
    $stats['hoy'] = (int)$pdo->query("SELECT COUNT(*) FROM fs_ventas WHERE DATE(fecha_venta) = CURDATE() AND estado = 'completada'")->fetchColumn();
    $stats['mes'] = (int)$pdo->query("SELECT COUNT(*) FROM fs_ventas WHERE MONTH(fecha_venta) = MONTH(CURDATE()) AND YEAR(fecha_venta) = YEAR(CURDATE()) AND estado = 'completada'")->fetchColumn();
    $stats['total_dinero'] = (float)$pdo->query("SELECT COALESCE(SUM(total), 0) FROM fs_ventas WHERE estado = 'completada'")->fetchColumn();
    
    // Estadísticas para dashboard
    $stats['ventasHoy'] = (float)$pdo->query("SELECT COALESCE(SUM(total), 0) FROM fs_ventas WHERE DATE(fecha_venta) = CURDATE() AND estado = 'completada'")->fetchColumn();
    $stats['ventasMes'] = (float)$pdo->query("SELECT COALESCE(SUM(total), 0) FROM fs_ventas WHERE MONTH(fecha_venta) = MONTH(CURDATE()) AND YEAR(fecha_venta) = YEAR(CURDATE()) AND estado = 'completada'")->fetchColumn();
...
function crearVenta($pdo, $auditoria) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO fs_ventas 
            (numero_venta, cliente_id, cliente_nombre, productos, subtotal, descuento, total, metodo_pago, estado, observaciones, id_usuario) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
...
function actualizarVenta($pdo, $auditoria, $id) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT * FROM fs_ventas WHERE id = ?");
        $stmt->execute([$id]);
...
function eliminarVenta($pdo, $auditoria, $id) {
    try {
        ...
        $stmt = $pdo->prepare("DELETE FROM fs_ventas WHERE id = ?");
        $stmt->execute([$id]);
